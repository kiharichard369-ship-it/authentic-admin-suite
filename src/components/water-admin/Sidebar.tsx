...
import {
  LayoutDashboard, Receipt, Package, Users, ClipboardList, Wallet,
  FileText, Droplets, LogOut, UserCircle, RefreshCcw, ShieldCheck,
  TrendingUp, GitBranch, CreditCard,
} from "lucide-react";
...
const items: Item[] = [
  ...
  // Admin-only
  { to: "/water-admin/branches",  label: "Branches",       icon: GitBranch,       perm: "water.branches" },
  { to: "/water-admin/payments",  label: "Payment Setup",  icon: CreditCard,      perm: "water.payments" },
];

export function WaterAdminSidebar({ role }: { role: Role }) {
  const path    = useRouterState({ select: (s) => s.location.pathname });
  const session = { role, email: "", name: "", vendorId: null, vendorName: null, businessType: null, branchId: null, branchName: null };
        ...
        <Link to="/water-admin/dashboard" className="flex items-center gap-2">
          ...
          <div>
            ...
            <div className="text-xs text-sidebar-foreground/60 mt-0.5">
              {role === "water_branch_manager" ? "Branch Workspace" : "Admin Workspace"}
            </div>
          </div>
        </Link>
      </div>
