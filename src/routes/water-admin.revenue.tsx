...
import {
  waterKpis as _mock_waterKpis, branch as _mock_branch, products as mockProducts,
} from "@/lib/water-mock";
import {
  fetchWaterKpis, fetchBranch, listProducts, fetchWaterRevenueSplit,
} from "@/lib/water-data";
...
const mockRevenueSplit = (() => {
  const total  = _mock_waterKpis.todayRevenue;
  const mpesa  = Math.round(total * 0.65);
  return { mpesa, cash: total - mpesa, total };
})();
...
function RevenuePage() {
  ...
  const revenueSplit = useLive(["water", "revenueSplit"]  as const, fetchWaterRevenueSplit,  mockRevenueSplit);
  ...
  return (
    <div>
      ...
      <div className="grid gap-4 md:grid-cols-3 mb-6">
        <Tile icon={Smartphone} label="M-Pesa revenue"  value={fmt(revenueSplit.mpesa)} />
        <Tile icon={Banknote}   label="Cash revenue"    value={fmt(revenueSplit.cash)} />
        <Tile icon={TrendingUp} label="Combined total"  value={fmt(revenueSplit.total)} highlight />
      </div>
      ...
      <div className="grid gap-4 md:grid-cols-2">
        <Card>
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground mt-1">Estimated inventory value at 07:00</p>
          </CardContent>
        </Card>
        <Card>
